...
const vue = new Vue({
  ...
  methods: {
    getTasks() {
      ...
      fetch(
        `${
          this.apiURL + this.apiVersion + route + this.backendSuffix
    ...
    deleteTask(index) {
      const request = JSON.stringify({ id: index });
      const route = this.apiVersion === 'v1' ? '/items' : '/router';
    ...
    addTask() {
      if (this.new_task.text.trim() !== '') {
        const request = JSON.stringify({ text: this.new_task.text });
        const route = this.apiVersion === 'v1' ? '/items' : '/router';
    ...
    logIn() {
      if (this.login.trim() !== '' && this.pass.trim()) {
        const params = JSON.stringify({ login: this.login, pass: this.pass });
